...
parser.add_option("--TableType", type="string", dest="tableType", default="BkgEst",help="choose the table type")
...
if options.tableType=="BinByBinYields":
    ...
    for ich,fname in enumerate(args):
        label = "ch%d" % (ich+1)
        if "=" in fname: (label,STString,RDString,fname) = fname.split("=")
        fname = options.fprefix+fname
        dirname = os.path.dirname(fname)
        file = open(fname, "r")
        DC = parseCard(file, options)
        SignalDict[label]=DC
        SignalConfDict[(label,"STString")]=STString
        SignalConfDict[(label,"RDString")]=RDString
    ...
    CollectYieldDicts = []
#    print SignalDict.keys()[0], SignalDict[SignalDict.keys()[0]], SignalDict[SignalDict.keys()[0]].bins
    FirstEntryKey=SignalDict.keys()[0]
    for b in SignalDict[FirstEntryKey].bins:
#        print b
        BinsToSelectAny=["_1B_","_2B_","_3p_"]
        BinsToSelectAll=["CnC2015X"] #e.g. ,"9Infj"] to select only highest bins
        if any(x in b for x in BinsToSelectAny) and all(x in b for x in BinsToSelectAll):
#            print [ [(key+"Bkg",0), (key+"Sig",0)] for key in SignalDict ]

#            print [ ("Bin",b)] + [ [(label+"Bkg",0), (label+"Sig",0)] for key in SignalDict]
            YieldDict = OrderedDict( [ ("Bin",b)] + list(itertools.chain.from_iterable([ [(key+"Bkg",0), (key+"Sig",0), (key+"SovB",0)] for key in SignalDict ])) )
#            print YieldDict

            for key in SignalDict:
                renamedBin = b.replace(SignalConfDict[(FirstEntryKey,"STString")],SignalConfDict[(key,"STString")])
                renamedBin = renamedBin.replace(SignalConfDict[(FirstEntryKey,"RDString")],SignalConfDict[(key,"RDString")])
#                print key, renamedBin, SignalDict[key].obs[renamedBin]
                YieldDict[key+"Bkg"]=SignalDict[key].obs[renamedBin]

                for (p,e) in SignalDict[key].exp[renamedBin].items(): # so that we get only self.DC.processes contributing to this bin
                    if SignalDict[key].isSignal[p]:
                        YieldDict[key+"Sig"]= e
                        if YieldDict[key+"Bkg"]!=0:
                            YieldDict[key+"SovB"]= e/YieldDict[key+"Bkg"]


            #DC_Bkg.obs[b]           
#            YieldDict[]=DC_Bkg.obs[b]
